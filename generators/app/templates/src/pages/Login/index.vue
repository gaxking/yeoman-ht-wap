...
<template>
  <div class="wrapper relative" :style="{minHeight:minHeight+'px'}">
    ...
    <div class="loginForm">
      ...
      <div v-else>
        <group title="">
          <x-input placeholder="请输入手机号" v-model="user.mobile"></x-input>
          <x-input placeholder="请输入密码"  @on-change="change" type="password" v-model="user.password"></x-input>
        </group>
    ...
    <div v-transfer-dom>
      <popup v-model="showAgree" position="bottom" class="agreeDialog" height="100%">
        <div class="relative h_100per">
          <x-header title="密蜜家居商品注册协议" :left-options="{backText: '',preventGoBack:true}" @on-click-back="showAgree=false"/>
...
export default {
  ...
  methods: {
    change(){
      ...
      console.log( loginType, this.loginBtnDisabled);
    },
    /*登录*/
    login(){
      let code = this.code;
